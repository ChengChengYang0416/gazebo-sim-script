...
## Bash script for setting up ROS Kinetic (with Gazebo 7) development environment for PX4 on Ubuntu LTS (16.04). 
...
## - MAVROS
  ...
  #echo "ROS Kinetic requires at least Ubuntu 16.04."
  #echo "Exiting ...."
  #return 1;
#fi
...
if ! rosdep install --from-paths src --ignore-src -y; then
    ...
    rosdep install --from-paths src --ignore-src --rosdistro kinetic -y --os ubuntu:bionic
fi
